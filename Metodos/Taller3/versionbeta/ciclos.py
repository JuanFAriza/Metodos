...
for i in range(len(transformadas)): #Halla la transformada
    ...
    for m in range(len(transformadas[i])):
        ...
        Pm = np.sum(conc*np.exp(-1.0j*m*2*np.pi*tiempos/T))
...
for i in range(len(transformadas)): #Busco los componentes mas fuertes, teniendo en cuenta que la amplitud de ciertas (las que no son N/2) frecuencias w, se dividen en dos en w y -w
    indicesMax.append([0,0])
    for j in range(2,int(len(transformadas[i])/2)):
        if abs(transformadas[i][j]) > abs(transformadas[i][indicesMax[i][0]]):
            indicesMax[i][0] = j
    for j in range(2,indicesMax[i][0])+range(indicesMax[i][0]+1,int(len(transformadas[i])/2)):
        if abs(transformadas[i][j]) > abs(transformadas[i][indicesMax[i][1]]):
            indicesMax[i][1] = j

fileout = open("periodos.dat", "w")
for i in range(len(transformadas)):
    indmax = np.array(indicesMax[i])
    horas = concentraciones[i][-1][0]+1
    periodosMax = horas/(24*indmax)

    fileout.write("%f %f\n"%(periodosMax[0],periodosMax[1]))
fileout.close()
